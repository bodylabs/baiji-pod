class PackRunner(object):
    ...
    def _parse_args(self):
        ...
        if self.default_vc_bucket is None:
            subparsers['dump'].add_argument('--vc_bucket', required=True, type=str, help='S3 bucket name for VC paths')
        else:
            subparsers['dump'].add_argument('--vc_bucket', default=None, type=str, help='S3 bucket name for VC paths; defaults to {}'.format(self.default_vc_bucket))

        if self.default_vc_manifest_path is None:
            subparsers['dump'].add_argument('--vc_manifest', required=True, type=str, help='S3 VC manifest')
        else:
            subparsers['dump'].add_argument('--vc_manifest', default=None, type=str, help='S3 VC manifest; defaults to {}'.format(self.default_vc_manifest_path))

        subparsers['dump'].add_argument('file', help='YAML file containing what to pack')
        subparsers['dump'].add_argument('save_to', type=str, default=None, help='Location to save the package')
        subparsers['dump'].add_argument('--max_size', type=int, default=None, help='max size of the packaged zip files, in MB')

        subparsers['load'].add_argument('files', type=str, nargs='+', help='zip files to unpack into the sc cache')
